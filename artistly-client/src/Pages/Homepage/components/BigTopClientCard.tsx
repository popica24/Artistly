...
const BigTopClientCard = (props: Props) => {
  ...
  return (
    <div
      ...
      className="bg-[#F3F3F3] rounded-[1.50894rem] flex flex-row justify-start items-center w-full cursor-pointer"
      style={containerStyle}
    >
      <div style={coverStyle} />
      <div className="flex flex-col items-start justify-start ps-10">
        <div className="flex flex-row items-center justify-center">
          <div className="rounded-full" style={profilePicStyle} />
          <div className="flex flex-col items-start justify-start ms-2">
            <span className="text-[#969696] font-medium uppercase text-[0.625rem] leading-5">
              {props.client.clientTag}
            </span>
            <span className="font-medium text-[1.5rem] leading-4">
              {props.client.clientName}
            </span>
          </div>
        </div>
        <span className="max-w-[40ch] text-[0.75rem] font-medium pb-12 pt-3">
          {props.client.clientDescription.length > 200
            ? props.client.clientDescription.substring(0, 200) + "..."
